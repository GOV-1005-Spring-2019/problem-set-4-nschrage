...
x %>% 
  ...
  #spreead the data to get parties
...
## Question #3
```{r 3, echo=FALSE}
#filter different education levels than relevel
x %>% 
...
group_by(educ) %>% 
#set up plot, style
ggplot(aes(x = educ, y = final_weight)) + coord_flip() + geom_violin() + geom_jitter(alpha = .5) + theme_gray()+ 
...
x %>% 
  ...
  filter(response %in% c("Dem", "Rep")) %>% 
  #set up graph
  ggplot(aes(fill = response, x = educ, y = value)) + geom_col(position = "dodge2") +
  facet_wrap(~response) %>% 
  labs(title = "% of North Carolina District 9 Polled Voters Education by Party", caption = "Theme: economist theme package, Data: Source: New York Times Upshot/Siena College") + 
  xlab(element_blank()) +
  ylab("Percentage") +
  theme_economist() +
